...
use sentiric_contracts::sentiric::sip::v1::{
    b2bua_service_server::B2buaService, InitiateCallRequest, InitiateCallResponse,
...
impl B2buaService for MyB2BuaService {
    ...
    async fn initiate_call(
        ...
        match self
            .engine
            .send_outbound_invite(&call_id, &req.from_uri, &req.to_uri)
            .await
        {
            Ok(_) => Ok(Response::new(InitiateCallResponse {
                success: true,
                new_call_id: call_id,
            })),
            Err(e) => {
    ...
    async fn transfer_call(
        &self,
        _req: Request<TransferCallRequest>,
    ) -> Result<Response<TransferCallResponse>, Status> {
        // [ARCH-COMPLIANCE] ARCH-007
        warn!(
            event = "RPC_TRANSFER_CALL_UNSUPPORTED",
            "⚠️ TransferCall henüz desteklenmiyor."
        );
