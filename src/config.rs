...
impl AppConfig {
    pub fn load_from_env() -> Result<Self> {
        let grpc_port =
            env::var("SIP_B2BUA_SERVICE_GRPC_PORT").unwrap_or_else(|_| "13081".to_string());
        let http_port =
            env::var("SIP_B2BUA_SERVICE_HTTP_PORT").unwrap_or_else(|_| "13080".to_string());
        let sip_port_str =
            env::var("SIP_B2BUA_SERVICE_SIP_PORT").unwrap_or_else(|_| "13084".to_string());
        ...
        let proxy_target = env::var("SIP_PROXY_SERVICE_SIP_TARGET")
            .unwrap_or_else(|_| "sip-proxy-service:13074".to_string());
        let sbc_target = env::var("SIP_SBC_SERVICE_SIP_TARGET")
            .context("ZORUNLU: SIP_SBC_SERVICE_SIP_TARGET")?;
        let sbc_public_ip =
            env::var("SIP_SBC_SERVICE_PUBLIC_IP").context("ZORUNLU: SIP_SBC_SERVICE_PUBLIC_IP")?;

        let public_sip_port = env::var("SIP_SBC_ADVERTISED_PORT")
        ...
        Ok(AppConfig {
            ...
            media_service_url: env::var("MEDIA_SERVICE_TARGET_GRPC_URL")
                .context("MEDIA_SERVICE_URL")?,

            proxy_service_url: env::var("SIP_PROXY_SERVICE_TARGET_GRPC_URL").unwrap_or_default(),
            registrar_service_url: env::var("SIP_REGISTRAR_SERVICE_TARGET_GRPC_URL")
                .context("REGISTRAR_URL")?,
