...
use crate::sip::store::{CallSession, CallSessionData, CallState, CallStore};
use sentiric_contracts::sentiric::dialplan::v1::ResolveDialplanRequest;
use sentiric_sip_core::{
    builder::SipResponseFactory, transaction::SipTransaction, Header, HeaderName, SipPacket, SipUri,
};
...
impl CallHandler {
    ...
    pub async fn process_invite(
        ...
        let call_id = req
            ...
            .unwrap_or_default();
        let from = req
            .get_header_value(HeaderName::From)
            .cloned()
            .unwrap_or_default();
        let to = req
            .get_header_value(HeaderName::To)
            .cloned()
            .unwrap_or_default();

        let caller_tag = from.split(";tag=").nth(1).unwrap_or("").to_string();
        let client_contact = req
            .get_header_value(HeaderName::Contact)
            .cloned()
            .unwrap_or_else(|| format!("<{}>", from));
        let to_uri = SipUri::from_str(&to)
            .unwrap_or_else(|_| SipUri::from_str("sip:unknown@sentiric.local").unwrap());
        let to_aor = to_uri.user.unwrap_or_default();

        let _ = transport
            .send(
                &SipResponseFactory::create_100_trying(&req).to_bytes(),
                src_addr,
            )
            .await;
        ...
        match dialplan_res {
            Ok(response) => {
                ...
                let rtp_port = match self.media_mgr.allocate_port(&call_id).await {
                    ...
                    Err(e) => {
                        ...
                        let _ = transport
                            .send(
                                &SipResponseFactory::create_error(&req, 503, "Media Error")
                                    .to_bytes(),
                                src_addr,
                            )
                            .await;
                ...
                let sbc_rtp_target = self
                    .extract_rtp_target_from_sdp(&req.body)
                    .unwrap_or_else(|| format!("{}:{}", src_addr.ip(), 30000));

                let _ = self
                    .media_mgr
                    .set_target(rtp_port, &sbc_rtp_target, &call_id)
                    .await;
                ...
                ok_resp
                    .headers
                    .push(Header::new(HeaderName::Contact, contact_uri));
                ok_resp.headers.push(Header::new(
            ...
            Err(e) => {
                ...
                let _ = transport
                    .send(
                        &SipResponseFactory::create_error(&req, 500, "Routing Error").to_bytes(),
                        src_addr,
                    )
                    .await;
            }
    ...
    pub async fn process_reinvite(
        ...
        if let Some(mut session) = session_opt {
            ...
            let _ = transport
                .send(
                    &SipResponseFactory::create_error(&req, 481, "Call Does Not Exist").to_bytes(),
                    src_addr,
                )
                .await;
        }
    ...
    pub async fn process_outbound_invite(
        &self,
        transport: Arc<sentiric_sip_core::SipTransport>,
        call_id: &str,
        from_uri: &str,
        ...
        let sdp_body = self.media_mgr.generate_sdp(rtp_port);
        let mut invite =
            SipPacket::new_request(sentiric_sip_core::Method::Invite, to_uri.to_string());
        let local_tag = sentiric_sip_core::utils::generate_tag("b2bua-out");

        invite.headers.push(Header::new(
            HeaderName::From,
            format!("<{}>;tag={}", from_uri, local_tag),
        ));
        invite
            .headers
            .push(Header::new(HeaderName::To, format!("<{}>", to_uri)));
        invite
            .headers
            .push(Header::new(HeaderName::CallId, call_id.to_string()));
        invite
            .headers
            .push(Header::new(HeaderName::CSeq, "1 INVITE".to_string()));
        let contact_uri = format!("<sip:b2bua@{}:{}>", self.config.sbc_public_ip, 5060);
        invite
            .headers
            .push(Header::new(HeaderName::Contact, contact_uri.clone()));
        invite.headers.push(Header::new(
            HeaderName::ContentType,
            "application/sdp".to_string(),
        ));
    ...
    pub async fn process_bye(
        ...
        let _ = transport
            .send(
                &SipResponseFactory::create_200_ok(&req).to_bytes(),
                src_addr,
            )
            .await;
        if let Some(session) = self.calls.remove(&call_id).await {
            self.media_mgr
                .release_port(session.data.rtp_port, &call_id)
                .await;
            self.event_mgr
                .publish_call_ended(&call_id, "normal_clearing")
                .await;
    ...
    pub async fn process_cancel(
        ...
        if let Some(session) = self.calls.remove(&call_id).await {
            self.media_mgr
                ...
                .await;
            self.event_mgr
                .publish_call_ended(&call_id, "cancelled")
                .await;
    ...
    pub async fn process_ack(&self, call_id: &str) {
        self.calls
            .update_state(call_id, CallState::Established)
            .await;
    ...
    pub async fn terminate_session(
        ...
        if let Some(session) = self.calls.remove(call_id).await {
            self.media_mgr
                ...
                .await;
            self.event_mgr
                .publish_call_ended(call_id, "system_terminated")
                .await;
            ...
            bye.headers.push(Header::new(
                HeaderName::Via,
                format!(
                    "SIP/2.0/UDP {}:{};branch={};rport",
                    self.config.public_ip, self.config.sip_port, branch
                ),
            ));
            bye.headers
                .push(Header::new(HeaderName::MaxForwards, "70".to_string()));
            ...
            bye.headers.push(Header::new(
                HeaderName::From,
                format!("{};tag={}", session.data.to_uri, session.data.local_tag),
            ));
            bye.headers
                .push(Header::new(HeaderName::CallId, call_id.to_string()));
            bye.headers
                .push(Header::new(HeaderName::CSeq, "999 BYE".to_string()));
            bye.headers.push(Header::new(
                HeaderName::Route,
                format!("<sip:sbc@{}:{};lr>", self.config.sbc_public_ip, 5060),
            ));
            bye.headers
                .push(Header::new(HeaderName::ContentLength, "0".to_string()));
